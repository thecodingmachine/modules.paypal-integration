<?php
namespace Mouf\Modules\PaypalServices;
...
class PaypalService {
	...
	*/
	...
	private function hasTransactionBeenProcessedBefore() {
		...
		if ($txn_id != null) {
			$ipn_response = $this->tdbmService->getObjects("paypal_ipn_responses", array(new EqualFilter("paypal_ipn_responses", "hackattempt", 0), new TDBM_EqualFilter("paypal_ipn_responses", "paypal_txn_id", $txn_id)));
